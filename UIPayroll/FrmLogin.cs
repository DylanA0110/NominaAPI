...
namespace UIPayroll
{
    public partial class FrmLogin : Form
    {
        ...
        private async Task LoginAsync()
        {
            ...
            var token =
                await _apiClient.UserRepo.AuthenticateUserAsync(username, password);
            if (!string.IsNullOrEmpty(token))
            {
                ...
                Hide();
                var frmPrincipal = new FrmPrincipal(_apiClient);
            }
            else
            {
                loginAttempts++;
                ...
                {
                    MessageBox.Show("Maximum login attempts reached. Closing application.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Application.Exit();
                }
                else
                {
